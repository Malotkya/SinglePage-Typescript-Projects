 ...
 * 
 */
export function initSound():InitalizeResult<undefined> {
    ...
    return Failure(new Error("Unable to load AudioContext!  No sound will be available!"));
 ...
 */
export function playFile(audio:Encoding|Uint16Array) {
    const ctx = _assert();
 ...
 * @param {Encoding|Uint16Array} audio 
 */
export function playSound(audio:Encoding|Uint16Array, sampleRate:number = 4) {
    const ctx = _assert();
...
export function getSound():SoundContext|null {
    return ctx;
}
